...
def exploitChallenge(p):
    ...
    # Key is automatically sent first 
    if len(sys.argv) > 1:
    ...
    p.write(flagFile2 + b"\x00"*(32-len(flagFile2)))
    sleep(1)
    flag2 = p.recv(512, timeout=5).rstrip(b'\0')
    
    with open("./recflag.enc", 'wb') as f:
        f.write(flag2)
    subprocess.check_call(f"openssl enc -d -a -aes-256-cbc -pbkdf2 -in ./recflag.enc -out ./flag2.txt -k {key.decode('utf-8')}".split(' '))
