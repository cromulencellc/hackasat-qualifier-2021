...
void CIO_ReadInterrupt( void )
{
	...
	uint32_t in_byte = (*READ_DATA_REG);	// Read clears interrupt

	uint16_t checksum;
	// debug_log(LOG_PRIORITY_HIGH, "read_buffer_byte %u\n", in_byte);

	if ( (*g_iodata).m_messageReceived == 1)
		return;

	if ( (*g_iodata).m_frameSync1Found == 0) {

		if (in_byte == 0xa5 ) {

			(*g_iodata).m_frameSync1Found = 1;
		}

		return;
	}

	if ( (*g_iodata).m_frameSync2Found == 0) {

		if (in_byte == 0x5a ) {

			(*g_iodata).m_readBufferWritePos = 0;
			(*g_iodata).m_frameSync2Found = 1;
			checksum = 0;
		}
		else if ( in_byte != 0xa5 ) {
	...
	(*g_iodata).m_readBuffer[(*g_iodata).m_readBufferWritePos++] = in_byte;

	checksum += in_byte;

	if ( (*g_iodata).m_readBufferWritePos == 62) {

		// debug_log(LOG_PRIORITY_HIGH, "chksum = %x", checksum);

		(*g_iodata).m_frameSync1Found = 0;
		(*g_iodata).m_frameSync2Found = 0;

		if (0xff == (checksum & 0xff))
			(*g_iodata).m_messageReceived = 1;
		else
			debug_log(LOG_PRIORITY_HIGH,"checksum failed");
...
void CIO_WriteInterrupt( void )
{
	...
	if ( (*g_iodata).m_writeBufferSize <= 0 )
	{
	...
	(*g_iodata).m_writeBufferSize--;
}

CIOConnection::CIOConnection( )
	: m_readBufferSize(0), m_readBufferReadPos(0), m_readBufferWritePos(0), m_writeBufferSize(0), m_writeBufferReadPos(0), m_writeBufferWritePos(0), m_frameSync1Found(0), m_frameSync2Found(0), m_messageReceived(0)
{
