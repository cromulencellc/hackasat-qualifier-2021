...
bool vmips::setup_clockdevice()
{
	...
	boot_msg( "Mapping clock %s to physical address 0x%08x\n",
		  clock_device->descriptor_str(), CLOCK_BASE );

	intc->connectLine( clock_irq, clock_device );
	boot_msg( "Connected clock %s to the %s\n", DeviceInt::strlineno(clock_irq),
		  clock_device->descriptor_str() );

	return true;
}

bool vmips::setup_sensordevice()
{
#if RUN_CHALLENGE_EMULATOR

	sensor_device = new SensorDevice();

	if ( physmem->map_at_physical_address( sensor_device, SENSOR_DEVICE_BASE ) == -1 )
	{
		error( "Failed to map sensor_device" );
		return false;
	}

	boot_msg( "Mapping %s to physical address 0x%08x\n",
		  sensor_device->descriptor_str(), SENSOR_DEVICE_BASE );
...
bool vmips::setup_transmitterdevice()
{

	transmitter_device = new TransmitterDevice();

	if ( physmem->map_at_physical_address( transmitter_device, TRANSMITTER_BASE ) == -1 )
	{
		error( "Failed to map transmitter_device" );
		return false;
	}
...
bool vmips::setup_synovauart1device()
{
	...
	int fd = creat("Logging.txt", S_IRWXU);

	// Byte mode, no read, write to stderr (FD # = 2)
	synova_uart1 = new SynovaUartDevice( false, true, -1, fd, 0, write_irq, false );
...
bool vmips::setup_synovauart2device()
{
	...
	synova_uart2 = new SynovaUartDevice( true, false, 0, 1, read_irq, write_irq, false );
