...
#define TICK_COUNTER	(6000)	// 200kbit/s sending 32-bits at a time at a clock speed of 10MHz
...
void SynovaUartDevice::uart_tick( void )
{
	...
	if ( enable_read && read_tick_count >= TICK_COUNTER )
	{
		...
		if ( read_count > 0 && FD_ISSET( read_fd, &read_set ) )
		{
			...
			//fprintf(stderr, "uart read data\n");

			// Data available
			ssize_t read_count = read( read_fd, &read_byte, 1 );

			// fprintf(stderr, "read_byte = %x\n", read_byte);
			...
			if ( read_count == 1 )
			{
				...
				else
				{
					...
					if ( read_interrupt_enable )
					{
						...
						// fprintf(stderr, "enabling uart read irq %x\n", read_irq);
						assertInt( read_irq );
						read_interrupt_set = true;
					}
					else {

						fprintf(stderr, "read_interrupt is not enabled\n");
					}
				}
			}
			else
...
uint32 SynovaUartDevice::fetch_word( uint32 offset, int mode, DeviceExc *client)
{
	...
	case 1:		// Read UART data
		if ( read_value_ready )
		{
			...
			{
				// Clear on read
				// fprintf(stderr, "clearing read_irq\n");
